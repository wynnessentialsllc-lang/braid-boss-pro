...
export async function POST(req: Request) {
  ...
  const { data: order, error: orderErr } = await admin
    .from("product_orders")
    .select(
      "id, user_id, status, fulfillment_status, shipping_rate_id, shipping_carrier, shipping_service, label_url, label_purchased_at, tracking_number, tracking_url, customer_email, customer_name, line_items, currency, amount_total",
    )
    .eq("id", orderId)
    .maybeSingle();

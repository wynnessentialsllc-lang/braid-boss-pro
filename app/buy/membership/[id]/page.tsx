...
// Public "join a membership" page. Anon. The stylist shares
// /buy/membership/<template_id>; the buyer enters their name + email and
// is sent to Stripe Checkout in subscription mode (a recurring charge on
// the stylist's connected account). The membership + each cycle's grant
// are written by the membership webhook.
...
const cadenceWord = (i: string | null) => (i === "week" ? "week" : i === "year" ? "year" : "month");
const perSuffix = (i: string | null) => (i === "week" ? "/wk" : i === "year" ? "/yr" : "/mo");
...
export default function BuyMembershipPage() {
      ...
      <Wrap>
        <Card>
          <div style={{ textAlign: "center" }}>
            ...
            <h1 style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 26, margin: "0 0 8px" }}>You&apos;re a member.</h1>
            <p style={{ margin: 0, color: C.coffee, fontSize: 14, lineHeight: 1.55 }}>
              Your membership is active. Your stylist will see it, and you&apos;ll get fresh visits or credit every cycle automatically.
            </p>
          </div>
        </Card>
      </Wrap>
  ...
  if (err === "not_found" || !tpl) {
    return (
      <Wrap><Card>
        <h1 style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 24, margin: "0 0 10px" }}>Membership not found</h1>
        <p style={{ margin: 0, color: C.coffee, fontSize: 14, lineHeight: 1.55 }}>This link is no longer available. Reach out to your stylist.</p>
      </Card></Wrap>
    );
  }

  // Guard: if a one-time package link is opened here, point the buyer to
  // the right page rather than starting a (failing) subscription.
  if (tpl.billing_mode !== "recurring") {
    return (
      <Wrap><Card>
        <h1 style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 24, margin: "0 0 10px" }}>This is a one-time package</h1>
        <p style={{ margin: 0, color: C.coffee, fontSize: 14, lineHeight: 1.55 }}>
          Open it at <span style={{ fontWeight: 600 }}>/buy/package/{tpl.id}</span>, or ask your stylist for the right link.
        </p>
      </Card></Wrap>
    );
  }

  const cadence = cadenceWord(tpl.billing_interval);
  const grant = tpl.kind === "visits"
  ...
  return (
    <Wrap>
      <div style={{ textAlign: "center", marginBottom: 18 }}>
        ...
        <p style={{ margin: "6px 0 0", fontSize: 13, color: C.muted }}>Membership · billed every {cadence}</p>
      </div>

      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${C.hairline}` }}>
          <span style={{ fontSize: 13, color: C.muted }}>You get</span>
          <span style={{ fontSize: 13, fontWeight: 700 }}>{grant} every {cadence}</span>
        </div>
        {tpl.service_label && (
          <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${C.hairline}` }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0" }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 800 }}>{money(tpl.price)}{perSuffix(tpl.billing_interval)}</span>
        </div>
        <p style={{ margin: "8px 0 0", fontSize: 11, color: C.muted, lineHeight: 1.5 }}>
          Renews automatically every {cadence}. Cancel anytime — you keep what you&apos;ve already paid for.
        </p>
      </Card>
      ...
      <Card>
        ...
        {!tpl.can_charge ? (
          ...
          <button onClick={join} disabled={busy}
            ...
            {busy ? "Starting checkout…" : `Join · ${money(tpl.price)}${perSuffix(tpl.billing_interval)}`}
          </button>
        )}
      </Card>

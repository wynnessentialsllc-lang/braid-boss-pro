-- Phase B5a — Booking approval state machine.
--
-- Replaces the binary pending/approved/declined flow with a five-state
-- machine that lets the stylist approve a request, send the client to
-- pay a deposit, and auto-confirm when the deposit lands. Phase B5a
-- ships the state machine + queue UI + slot holds + intelligence
-- metrics; Stripe wiring (deposit Checkout + webhook) lands in B5c.
--
-- States:
--   pending_review              — fresh request, awaiting stylist
--   approved_pending_deposit    — stylist approved; client must pay
--   confirmed                   — deposit captured; appointment locked
--   expired                     — approval window elapsed without payment
--   declined                    — stylist declined the request
--
-- The legacy `status` column is kept for backwards-compat — every
-- screen that read it still works. New flows write `approval_status`,
-- and the legacy column is mirrored so reports and notifications
-- stay sane while we migrate the UI.
...
-- Backfill approval_status from the legacy `status` column so every
-- existing row lands in a sensible state. Idempotent — only touches
-- rows where approval_status is still null.
update public.booking_requests
set approval_status = case
  when status = 'approved'  then 'confirmed'
  when status = 'converted' then 'confirmed'
...
create index if not exists booking_requests_approval_status_idx
  on public.booking_requests (user_id, approval_status, created_at desc);

create index if not exists booking_requests_holds_idx
  on public.booking_requests (user_id, preferred_date, approval_expires_at)
  where approval_status = 'approved_pending_deposit';

-- =====================================================================
-- 2. expire_stale_approvals — lazy cleanup
-- =====================================================================
-- Called from the client before any read that depends on fresh state
-- (queue list, public availability) so expirations propagate without
-- needing pg_cron. Owner-scoped; runs as the caller so RLS still
-- protects the row set we touch.
...
-- =====================================================================
-- 3. approve_booking_request — stylist approves + opens deposit window
-- =====================================================================
...
begin
  ...
  -- Resolve deposit. Explicit override wins, otherwise pull from the
  -- service snapshot stamped at request time.
  select coalesce(
    deposit_amount_in,
...
-- =====================================================================
-- 5. mark_booking_deposit_paid — manual fallback (B5a)
-- =====================================================================
-- B5c will replace the call site with the Stripe webhook. Keeping the
-- RPC owner-scoped here means the manual flow is itself a useful
-- escape hatch (e.g. the client paid in person) even after Stripe is
-- live. Idempotent: a second call on a confirmed row is a no-op.
...
begin
  ...
  set approval_status = 'confirmed',
      ...
      stripe_session_id = coalesce(stripe_session_id_in, stripe_session_id),
      approval_expires_at = null,
...
revoke all on function public.mark_booking_deposit_paid(uuid, text) from public;
grant execute on function public.mark_booking_deposit_paid(uuid, text) to authenticated;

-- =====================================================================
-- 6. public_list_availability — honour pending-deposit holds
-- =====================================================================
-- Re-creates the Phase B2 RPC with a third union in the `reserved`
-- CTE: any approved-pending-deposit request whose hold hasn't expired
-- blocks its preferred slot. Without this, two clients could both pay
-- the deposit for the same time. We match by exact preferred_date and
-- preferred_time only — flexible-time requests don't hold a slot.
...
begin
  ...
  reserved as (
    -- Confirmed bookings, padded by buffer.
    select
    ...
    union all
    -- Phase B5a — approved-pending-deposit holds. The slot is
    -- reserved as long as the approval window hasn't elapsed; a
    -- separate lazy-expire pass clears stale rows.
    select
...
-- =====================================================================
-- 7. get_booking_intelligence — extend with approval metrics
-- =====================================================================
-- Adds an `approvals` block alongside the existing seven sections so
-- the dashboard can surface deposit conversion rate + lost revenue
-- from expired approvals without a second round-trip.
...
begin
  ...
  -- Approvals block — Phase B5a metric set.
  with by_state as (

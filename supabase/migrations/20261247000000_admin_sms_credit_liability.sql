-- admin_command_center — surface unredeemed SMS credit liability.
--
-- SMS credits are prepaid: a stylist pays for a pack today and the
-- Twilio cost lands whenever they actually send, possibly months later
-- and possibly all at once. Cash arrives before cost, which flatters
-- early revenue and hides a real obligation -- every unspent credit is
-- a text the platform has already been paid to deliver.
--
-- The command center reported sms_revenue and sms_credits_sold (what
-- came in) with nothing for what is still owed. Adds:
--
--   sms_credits_outstanding      -- credits unspent across all accounts
--   sms_accounts_holding_credits -- how many stylists hold a balance
--
-- Deliberately NOT windowed: liability is a current-state balance, not
-- a flow. The dollar estimate is applied in the app, where the
-- per-segment cost sits beside the canonical pack pricing rather than
-- being frozen into a migration.
--
-- Every other field is unchanged from 20261205000000.
...
begin
  ...
  -- ---- Unredeemed SMS credits (current state, not windowed) -----------
  -- Credits are prepaid: the money is already collected but the Twilio
  -- cost lands whenever the stylist actually sends. This is the count
  -- still owed across every account -- the platform's outstanding
  -- delivery liability. The dollar estimate is applied in the app so
  -- the per-segment rate stays next to the pack pricing.
  sms_liability as (

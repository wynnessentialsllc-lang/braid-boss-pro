-- Fix the deposits-by-day sparkline reading 100x too small.
--
-- booking_requests.deposit_amount has always held DOLLARS (every other
-- consumer — north_star.deposited_revenue, revenue.deposits_collected,
-- revenue.deposits_at_booking — formats it straight through usd() with
-- no conversion). But the CTEs here named that summed value `cents`,
-- and the admin page's chart trusted the name: valueFormat={(cents) =>
-- usd(cents / 100)}. A real $25.00 deposit day rendered as $0.25.
--
-- Renames the misleading alias to `amount` end to end — the CTE
-- columns, and the JSON key the trend block emits (deposits_by_day[].
-- cents -> .amount) — so the field name can't mislead a formatter
-- again. north_star/revenue were already correct and are unchanged
-- except for the same internal rename.

-- Don't remind clients about appointments that were cancelled or deleted.
--
-- Bug: a client received a 24-hour reminder for an appointment that had
-- already been removed from the schedule. Root cause — the booking_requests
-- scan (Scan A) of both reminder functions guarded only with
--
--     if appt_status = 'cancelled' then continue; end if;
--
-- after `select status into appt_status from appointments where id = ...`.
-- Two ways that guard leaks:
--   1. If the linked appointment row has been hard-deleted from the
--      schedule, the SELECT matches no row: FOUND is false and appt_status
--      is left NULL (or stale from a prior loop iteration). `NULL = 'cancelled'`
--      is NULL, so the guard does not fire and the reminder is sent for an
--      appointment that no longer exists.
--   2. It only matched the literal 'cancelled', while the appointments scan
--      (Scan B) already excludes the full terminal set
--      ('cancelled','canceled','no-show','no_show','noshow','declined',
--      'completed').
--
-- Fix: in both functions, treat a missing appointment (NOT FOUND) and any
-- terminal status as "do not remind". The appointments scan is unchanged.
-- Both functions are reproduced verbatim from 20261021000000 with only the
-- Scan A guard tightened.

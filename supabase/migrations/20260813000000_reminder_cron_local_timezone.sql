-- Appointment-reminder cron now interprets the local start time in
-- the stylist's timezone (not naively as UTC).
--
-- Same class of bug fixed for review emails in the prior migration:
-- the cron cast (date || ' ' || time) to a naive `timestamp` and
-- compared it to now() (timestamptz). Postgres coerces naive →
-- session TZ (UTC), so a 9 AM PDT appointment had start_ts treated
-- as 9 AM UTC — 7h ahead of reality. The 18–30h window then opened
-- 7h too early, so a Saturday-morning booking could get a
-- "reminder" sent in the middle of Friday afternoon (or none at
-- all, depending on cron cadence).
--
-- Fix: resolve the stylist's local zone and use AT TIME ZONE so
-- start_ts is a real timestamptz. Resolution chain matches the
-- review-email migration so behavior is consistent across all
-- time-based crons:
--   Scan A (booking_requests):
--     1. booking_requests.timezone if set
--     2. most recent non-null booking_requests.timezone for the user
--     3. 'America/Los_Angeles' fallback
--   Scan B (stylist-created appointments):
--     1. appointments.timezone if set
--     2. most recent non-null booking_requests.timezone for the user
--     3. 'America/Los_Angeles' fallback
--
-- No structural change to the 18–30h window, throttle, opt-out
-- gates, or SMS branch — just the timestamp interpretation.

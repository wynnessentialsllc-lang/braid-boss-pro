-- SMS PR 2 — thread the booking-flow SMS opt-in through the
-- public booking submission RPC.
--
-- Adds a trailing sms_opt_in_in parameter (defaulted, so the named-
-- arg call from the booking page stays compatible) and stamps it
-- onto booking_requests.sms_opt_in. Drop + recreate because adding
-- a parameter changes the function identity.
...
begin
  ...
  select p.stripe_connect_account_id, p.stripe_connect_charges_enabled into owner_connect_id, owner_charges_enabled

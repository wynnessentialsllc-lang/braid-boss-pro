...
-- (sms_consent_ip, migration 20261004) to also retain the user-agent and
-- a consent-language version, and to tag the booking-form source as
-- "public_booking_page". All of it is captured server-side inside the
-- existing stamp_sms_consent() trigger from request headers + constants,
-- so no RPC signature changes are needed and a missing request context
-- can never block a write.
...
begin
  if NEW.sms_opt_in is true then
    ...
    -- Best-effort capture of client IP + user-agent from the PostgREST
    -- request headers. Wrapped so a missing/malformed request context
    -- (e.g. a direct SQL write) can never block the booking write.
    begin

-- Exclude rejected (fraud) accounts from the business-facing metrics on
-- the admin command center.
--
-- The Aug 25 fraud wave left 4 banned/rejected accounts sitting in
-- profiles with a real created_at, a connected Stripe account, and (for
-- most of them) an auto-started trial. Every one of those is still
-- counted in accounts-created, Stripe-connected, the subscription
-- breakdown, and the activation funnel — so "9 accounts / 8 trialing /
-- 5 Stripe connected" reads as active pipeline when 4 of those are
-- confirmed fraud that will never convert.
--
-- Revenue figures (deposits, booking fees, retail, SMS) are untouched —
-- those already only reflect real transactions, since the fraud never
-- touched booking_requests/appointments. This only filters the
-- profiles-derived counts: subs, subs_by_status, activation, and
-- stripe_health. `platform_review_status <> 'rejected'` (rather than
-- requiring 'approved') keeps legitimate pending signups in the funnel
-- — only confirmed-and-banned accounts drop out.
...
begin
  ...
  subs as (
    ...
    where platform_review_status <> 'rejected'
  ),
  subs_by_status as (
  ...
  activation as (
    select
      (select count(*) from public.profiles where platform_review_status <> 'rejected')::int as accounts,
      (select count(*) from public.profiles where stripe_connect_account_id is not null and platform_review_status <> 'rejected')::int as stripe_connected,
      (select count(*) from public.profiles where stripe_connect_charges_enabled is true and platform_review_status <> 'rejected')::int as charges_enabled,

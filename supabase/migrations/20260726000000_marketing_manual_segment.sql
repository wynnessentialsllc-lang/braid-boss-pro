...
--
-- Adds a fourth segment kind, `manual`, to the campaign composer:
--   { "kind": "manual", "client_ids": ["<id>", "<id>", ...] }
-- so a stylist can target specific clients instead of a bucket.
--
-- Only count_marketing_segment + process_marketing_campaign need
-- to learn the new kind — the recipient resolution is the only
-- place segment shape is interpreted. CREATE OR REPLACE keeps this
-- idempotent; both function signatures are unchanged.
--
-- Same guard rails as every other segment: opted-in clients with a
-- real email only. A manually-picked client who later unsubscribes
-- is still filtered out at send time — the picker captures intent,
-- the opt-out flag is always authoritative.
...
begin
  ...
  if v_kind = 'all' or v_kind is null then
    ...
    -- Count only the picked ids that are still send-eligible. A
    -- client unsubscribed since being picked drops out here, so the
    -- "Send to N" preview reflects who will actually receive it.
    select count(*) into v_count from public.clients c
      where c.user_id = caller
        and c.marketing_emails_enabled = true
        and c.email is not null and length(trim(c.email)) > 3

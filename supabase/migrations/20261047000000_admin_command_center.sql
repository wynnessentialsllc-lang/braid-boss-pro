-- admin_command_center — single aggregated business snapshot for the
-- platform owner's command center at /admin.
--
-- This complements analytics_summary_for_admin (event-stream analytics)
-- with the *business* KPIs from the product strategy: the North Star
-- (deposited booking revenue per active braider), subscriptions/MRR,
-- payments volume, the activation funnel, and live demand.
--
-- Security: SECURITY DEFINER so it can read across every braider's rows,
-- but it raises 'not_admin' unless the caller email matches the single
-- platform-owner allow-list (same gate as analytics_summary_for_admin and
-- app/lib/admin.ts → isAdminUser). The /api/admin/command-center route
-- additionally verifies the caller's JWT before this RPC is ever reached.
--
-- All monetary columns on appointments / booking_requests / product_orders
-- are numeric DOLLARS; sms_credit_purchases.amount_cents is integer cents.
-- The function returns dollars throughout (sms converted /100).
...
begin
  ...
  -- ---- Demand: requests + Build Your Style (AI quote) usage ------------
  requests_win as (
    select
      count(*)::int as total,
      ...
      count(*) filter (where coalesce(array_length(inspiration_photo_urls, 1), 0) > 0)::int as ai_quotes,
  ...
  deposits_by_day as (
    ...
    from public.booking_requests
    where deposit_paid is true and deposit_paid_at >= win_start
    group by 1
  ),
  bookings_by_day as (
  ...
  select jsonb_build_object(
    ...
    'bookings', jsonb_build_object(
      ...
      'ai_quote_requests',  (select ai_quotes from requests_win),
